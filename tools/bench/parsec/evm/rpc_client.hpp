...
class geth_client : public cbdc::rpc::json_rpc_http_client {
  public:
    ...
    static constexpr auto output_data_key = "output_data";
    ...
    /// Calls eth_getTransactionReceipt with the given transaction and
    /// callback function.
    /// \param tx transaction bytecode
    /// \param cb callback function passed to call()
    void get_transaction_receipt(const std::string& tx,
                          std::function<void(std::optional<std::string>)> cb);
