...
namespace cbdc::generate_config {
    ...
    void config_generator::set_log_level(const std::string& key,
        ...
        if(log_levels.find(log_level) == log_levels.end()) {
            log_level = "DEBUG";
            std::cerr << "Warning: Log level not recognized. Setting to DEBUG"
                      << std::endl;
        ...
        if(component_count == 0) {
            ...
            for(size_t i = 0; i < component_count; i++) {
                if(type == shard_count_key) {
                    ...
                    std::cerr << "Warning: Unrecognized component type, "
                              << type
            ...
            for(size_t i = 0; i < component_count; i++) {
                if(type == shard_count_key) {
                    ...
                    create_atomizer_sentinel(_default_log_level.value(), i);
    ...
    config_generator::create_2pc_shard(const std::string& default_log_level,
        ...
        shard_name += "_";
        shard_name += std::to_string(current_component_num);
    ...
    config_generator::create_2pc_sentinel(const std::string& default_log_level,
        ...
        auto public_key_key = sentinel_name + "_public_key";
    ...
    void config_generator::create_2pc_coordinator(
        ...
        coordinator_name += "_";
        coordinator_name += std::to_string(current_component_num);
    ...
    void config_generator::create_atomizer_sentinel(
        const std::string& default_log_level,
        size_t current_component_num) {
        auto sentinel_name
            = "sentinel" + std::to_string(current_component_num);
        ...
        for(auto const& dir_entry :
            ...
            if(tmp_str == match_str) {
                ...
                std::filesystem::copy(dir_entry,
                                      build_config_dir,
                                      copyOptions);
                std::cerr << "Copying " << dir_entry.path().string() << " to "
    ...
    [[nodiscard]] auto config_generator::find_value(const std::string& key)
        ...
        if(template_options.find(key) != template_options.end()) {
            ...
            std::cerr << "Warning: Unknown type for " << key
                      << " template parameter." << std::endl;
            return std::nullopt;
        }
        std::cerr << "Warning: Missing " << key << " template parameter."
                  << std::endl;
        return std::nullopt;
    }
    ...
    [[maybe_unused]] auto config_generator::generate_configuration_file()
        ...
        if(!m_template_file_is_valid) {
            ...
            m_new_config
                << "File provided, " + m_template_config_file
           ...
           && std::get<size_t>(is_two_phase_mode) == 1) {
            const auto coordinator_count
                = get_param_from_template_file(coordinator_count_key,
                                               config_map);
            m_new_config << create_component(sentinel_count_key,
                                             std::get<size_t>(sentinel_count),
                                             true);
            m_new_config << create_component(shard_count_key,
                                             std::get<size_t>(shard_count),
                                             true);
            m_new_config << create_component(
                coordinator_count_key,
            ...
            m_new_config << create_component(sentinel_count_key,
                                             std::get<size_t>(sentinel_count),
                                             false);
            m_new_config << create_component(archiver_count_key,
                                             std::get<size_t>(archiver_count),
                                             false);
            m_new_config << create_component(atomizer_count_key,
