 ...
 * self-contained abstraction without depending on other project files. For
 * this reason, Clang lifetimebound is defined here instead of including
 ...
 *   temporary. For example, this will compile, but exhibits undefined
 * behavior:
 *
 ...
 *   Thus the Span is left with a dangling reference, and using it is
 * undefined.
 *
 ...
 *
 *   This is very useful in cases where a function truly does not care about
 * the container, and only about having exactly a range of elements. However it
 ...
 *   result will be present in that variable after the call. Passing a
 * temporary is useless in that context.
 */
template<typename C>
class Span {
    C* m_data;
  ...
  public:
    ...
    template<
        typename T,
        typename std::enable_if<std::is_convertible<T (*)[], C (*)[]>::value,
                                int>::type
        = 0>
     ...
     *
     *  For example this means that a Span<T> can be converted into a
     * Span<const T>.
     */
    template<
        typename O,
        typename std::enable_if<std::is_convertible<O (*)[], C (*)[]>::value,
                                int>::type
        = 0>
    ...
    template<int N>
    constexpr Span(C (&a)[N]) noexcept : m_data(a),
                                         m_size(N) {}
        ...
        typename std::enable_if<
            !is_Span<V>::value
                && std::is_convertible<
                    ...
                    C (*)[]>::value
                && std::is_convertible<
                    decltype(std::declval<const V&>().size()),
                    std::size_t>::value,
            std::nullptr_t>::type
        = nullptr)
        : m_data(other.data()),
...
inline unsigned char* UCharCast(unsigned char* c) {
    return c;
}
inline const unsigned char* UCharCast(const char* c) {
    return (unsigned char*)c;
}
inline const unsigned char* UCharCast(const unsigned char* c) {
    return c;
}
...
template<typename T>
constexpr auto UCharSpanCast(Span<T> s) -> Span<
    typename std::remove_pointer<decltype(UCharCast(s.data()))>::type> {
