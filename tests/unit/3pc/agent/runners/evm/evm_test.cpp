...
class evm_test : public ::testing::Test {
  protected:
    ...
    std::shared_ptr<cbdc::telemetry> m_tel{
        std::make_shared<cbdc::telemetry>("telemetry.txt")};
...
TEST_F(evm_test, contract_deploy) {
    ...
    tx->m_sig = cbdc::threepc::agent::runner::evm_sig{};

    params = cbdc::make_buffer(
        cbdc::threepc::agent::runner::evm_dryrun_tx{m_addr0_addr, *tx});
...
TEST_F(evm_test, test_decode_tx_access_list) {
    ...
    auto tx_id
        = cbdc::threepc::agent::runner::tx_id(*tx, eth_mainnet_chain_id);
    auto expected_id_buf
        = cbdc::buffer::from_hex("2695ed62cf8cb7759d651c43dc28ffc1dd6a26103841"
                                 "c223721b081b55f4d0b5")
              .value();
