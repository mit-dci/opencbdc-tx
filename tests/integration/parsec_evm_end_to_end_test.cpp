...
#include "parsec/agent/runners/evm/http_server.hpp"
...
class parsec_evm_end_to_end_test : public ::testing::Test {
  protected:
    void SetUp() override {
        m_log->debug("parsec_evm_end_to_end_test::Setup()");
    ...
    std::unique_ptr<cbdc::parsec::agent::rpc::http_server> m_rpc_server;
...
void parsec_evm_end_to_end_test::init_accounts() {
    ...
    m_acct0_ethaddr = cbdc::parsec::agent::runner::eth_addr(m_acct0_privkey,
                                                            m_secp_context);
    m_acct1_ethaddr = cbdc::parsec::agent::runner::eth_addr(m_acct1_privkey,
                                                            m_secp_context);
...
void parsec_evm_end_to_end_test::init_jsonrpc_server_and_client(
    const cbdc::network::endpoint_t& rpc_server_endpoint) {
    ...
    m_broker = std::make_shared<cbdc::parsec::broker::impl>(
        0,
        shards,
    ...
    m_rpc_server = std::make_unique<cbdc::parsec::agent::rpc::http_server>(
...
void parsec_evm_end_to_end_test::test_erc20_tx_get_raw_output_data_(
    const evmc::address& contract_address,
    const evmc::address& from_address,
    const cbdc::privkey_t& from_privkey,
    const cbdc::buffer& input_data,
    std::string& out_raw_output_data) {
    const auto make_evmtx_ = [&]() -> cbdc::parsec::agent::runner::evm_tx {
        auto etx = cbdc::parsec::agent::runner::evm_tx();
        ...
        auto sighash = cbdc::parsec::agent::runner::sig_hash(etx);
        etx.m_sig = cbdc::parsec::agent::runner::eth_sign(from_privkey,
                                                          sighash,
                                                          etx.m_type,
                                                          m_secp_context);
        return etx;
    ...
    m_log->info(gtest_descr(),
                ...
                "From:",
                cbdc::parsec::agent::runner::to_hex(from_address),
                "Contract:",
    ...
    ASSERT_EQ(txreceipt["from"],
              "0x" + cbdc::parsec::agent::runner::to_hex(from_address));
...
void parsec_evm_end_to_end_test::test_erc20_name(
    const evmc::address& contract_address,
    const evmc::address& from_address,
    const cbdc::privkey_t& from_privkey,
    const std::string& expected_name) {
...
void parsec_evm_end_to_end_test::test_erc20_symbol(
    const evmc::address& contract_address,
    const evmc::address& from_address,
    const cbdc::privkey_t& from_privkey,
    const std::string& expected_symbol) {
...
void parsec_evm_end_to_end_test::test_erc20_decimals(
    const evmc::address& contract_address,
    const evmc::address& from_address,
    const cbdc::privkey_t& from_privkey,
    const evmc::uint256be& expected_decimals) {
    m_log->info(
        ...
        cbdc::parsec::agent::runner::to_hex_trimmed(expected_decimals));
    std::string raw_output_data;
    test_erc20_tx_get_raw_output_data_(
        contract_address,
        from_address,
    ...
    const auto decimals
        = cbdc::parsec::agent::runner::uint256be_from_hex(raw_output_data);
    ASSERT_TRUE(decimals.has_value());
    ASSERT_EQ(decimals.value(), expected_decimals);
}

void parsec_evm_end_to_end_test::test_erc20_total_supply(
    const evmc::address& contract_address,
    const evmc::address& from_address,
    const cbdc::privkey_t& from_privkey,
    const evmc::uint256be& expected_total_supply) {
...
void parsec_evm_end_to_end_test::test_erc20_deploy_contract(
    const evmc::address& from_addr,
    const cbdc::privkey_t& from_privkey,
    evmc::address& out_contract_address) {
    const auto make_evmtx_ = [&]() -> cbdc::parsec::agent::runner::evm_tx {
    ...
    const auto expected_contract_address
        = cbdc::parsec::agent::runner::contract_address(from_addr,
                                                        etx.m_nonce);
    m_log->info(
        gtest_descr(),
        std::string(__FUNCTION__) + "()",
...
void parsec_evm_end_to_end_test::test_erc20_get_balance(
    const evmc::address& contract_address,
    const evmc::address& acct_address,
    const cbdc::privkey_t& acct_privkey,
    const evmc::uint256be& expected_balance) {
    m_log->info(gtest_descr(),
                ...
                cbdc::parsec::agent::runner::to_hex(acct_address),
                "is",
                cbdc::parsec::agent::runner::to_hex_trimmed(expected_balance));
...
void parsec_evm_end_to_end_test::test_erc20_send_tokens(
    const evmc::address& contract_address,
    const evmc::address& from_address,
    const cbdc::privkey_t& from_privkey,
    ...
    m_log->info(gtest_descr(),
                ...
                cbdc::parsec::agent::runner::to_hex(from_address),
                "To:",
                cbdc::parsec::agent::runner::to_hex(to_address),
                "Contract:",
                cbdc::parsec::agent::runner::to_hex(contract_address));
...
TEST_F(parsec_evm_end_to_end_test, native_transfer) {
    ...
    const auto make_evmtx_ = [&]() -> cbdc::parsec::agent::runner::evm_tx {
        ...
        etx.m_gas_limit = evmc::uint256be(0xffffffff);
        auto sighash = cbdc::parsec::agent::runner::sig_hash(etx);
        etx.m_sig = cbdc::parsec::agent::runner::eth_sign(m_acct0_privkey,
                                                          sighash,
                                                          etx.m_type,
                                                          m_secp_context);
        return etx;
    ...
    ASSERT_TRUE(txreceipt.isMember("from"));
    ASSERT_EQ(txreceipt["from"],
              "0x" + cbdc::parsec::agent::runner::to_hex(m_acct0_ethaddr));

    ASSERT_TRUE(txreceipt.isMember("to"));
    ASSERT_EQ(txreceipt["to"],
              "0x" + cbdc::parsec::agent::runner::to_hex(etx.m_to.value()));
    ...
    ASSERT_EQ(sender_balance.value(),
              cbdc::parsec::agent::runner::operator-(m_init_acct_balance,
                                                     send_value));
...
TEST_F(parsec_evm_end_to_end_test, erc20_all) {
    ...
    test_erc20_decimals(
        ...
        m_acct0_ethaddr,
        m_acct0_privkey,
        cbdc::parsec::agent::runner::uint256be_from_hex("0x12").value());
    ...
    const auto txfer_amount
        = cbdc::parsec::agent::runner::uint256be_from_hex("0xF423F").value();
