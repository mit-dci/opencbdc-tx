...
namespace cbdc::generate_config {
    ...
    [[nodiscard]] auto config_generator::create_key_pair()
        -> std::pair<std::string, std::string> {
        if(m_randomize) {
    ...
    void config_generator::set_log_level(const std::string key,
        ...
        if(log_levels.find(log_level) == log_levels.end()) {
            ...
            std::cout << "Warning: Log level not recognized. Setting to DEBUG"
                      << std::endl;
        } else if(find_value<std::string>(key).has_value() == false) {
            log_level
    ...
    config_generator::create_component(const char* type,
                                       const value_t component_count,
                                       bool create_2pc) -> std::string {
        std::string return_msg = "";
        if(std::holds_alternative<size_t>(component_count)) {
            auto _default_log_level
                = find_value<std::string>(tmpl_default_log_level);
            if(std::get<size_t>(component_count) == 0) {
                return_msg += "Warning: 0 count for at least one component. "
                              "Fix configuration template and rerun.\n";
            } else if(create_2pc == true) {
                for(size_t i = 0; i < std::get<size_t>(component_count); i++) {
                    if(type == shard_count_key) {
                        // Create Shards
                        create_2pc_shards(_default_log_level.value(), i);
                    } else if(type == sentinel_count_key) {
                        // Create Sentinels
                        create_2pc_sentinels(_default_log_level.value(), i);
                    } else if(type == coordinator_count_key) {
                        // Create Coordinators
                        create_2pc_coordinators(_default_log_level.value(), i);
                    } else {
                ...
                for(size_t i = 0; i < std::get<size_t>(component_count); i++) {
                    if(type == shard_count_key) {
                        // Create Shards
                        create_atomizer_shards(_default_log_level.value(), i);
                    } else if(type == sentinel_count_key) {
                        // Create Sentinels
                        create_atomizer_sentienls(_default_log_level.value(),
                                                  i);
                    } else if(type == archiver_count_key) {
                        // Create Archiver(s)
                        create_atomizer_archivers(_default_log_level.value(),
                                                  i);
                    } else if(type == atomizer_count_key) {
                        // Create Atomizers
                        create_atomizer_atomizers(_default_log_level.value(),
                                                  i);
                    } else if(type == watchtower_count_key) {
                        // Create Watchtowers
                        create_atomizer_whatchtowers(
                            _default_log_level.value(),
                            i);
                    } else {
                        std::cout << "Warning: Unrecognized component type, "
                                  << type
    ...
    void
    config_generator::create_2pc_shards(const std::string default_log_level,
                                        size_t current_component_num) {
        std::string shard_name
            = "shard" + std::to_string(current_component_num);
        ...
        std::string read_only_endpoint_key = shard_name + "_readonly_endpoint";
        ...
        std::string log_level_val = default_log_level;
    ...
    void
    config_generator::create_2pc_sentinels(const std::string default_log_level,
                                           size_t current_component_num) {
    ...
    void config_generator::create_2pc_coordinators(
        ...
        std::string endpoint_val
            = cbdc::network::localhost + ":" + std::to_string(get_open_port());
        set_param_to_config_file(endpoint_key, endpoint_val);
        std::string raft_endpoint_key = coordinator_name + "_raft_endpoint";
        std::string raft_endpoint_val
            = cbdc::network::localhost + ":" + std::to_string(get_open_port());
        set_param_to_config_file(raft_endpoint_key, raft_endpoint_val);
    ...
    void config_generator::create_atomizer_shards(
        const std::string default_log_level,
        size_t current_component_num) {
        std::string shard_name
            = "shard" + std::to_string(current_component_num);
    ...
    void config_generator::create_atomizer_sentienls(
        const std::string default_log_level,
        size_t current_component_num) {
        std::string sentinel_name
            = "sentinel" + std::to_string(current_component_num);
    ...
    void config_generator::create_atomizer_archivers(
        const std::string default_log_level,
        size_t current_component_num) {
        std::string archive_name
            = "archiver" + std::to_string(current_component_num);
    ...
    void config_generator::create_atomizer_atomizers(
        const std::string default_log_level,
        size_t current_component_num) {
        std::string atomizer_name
    ...
    void config_generator::create_atomizer_whatchtowers(
        const std::string default_log_level,
        size_t current_component_num) {
        std::string watchtower_name
            = "watchtower" + std::to_string(current_component_num);
        std::string client_endpoint_key = watchtower_name + "_client_endpoint";
           ...
           && std::get<size_t>(is_two_phase_mode) == 1) {
            ...
            const auto coordinator_count
                = get_param_from_template_file(coordinator_count_key,
                                               config_map);
            return_msg
                += create_component(sentinel_count_key, sentinel_count, true);
            return_msg += create_component(shard_count_key, shard_count, true);
            ...
            const auto watchtower_count
                = get_param_from_template_file(watchtower_count_key,
                                               config_map);
            return_msg
                += create_component(shard_count_key, shard_count, false);
            return_msg
                += create_component(sentinel_count_key, sentinel_count, false);
            return_msg
                += create_component(archiver_count_key, archiver_count, false);
            return_msg
                += create_component(atomizer_count_key, atomizer_count, false);
