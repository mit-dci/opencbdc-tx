...
namespace cbdc::generate_config {
    ...
    class config_generator {
      ...
      private:
        ...
        [[nodiscard]] auto create_key_pair()
            -> std::pair<std::string, std::string>;
        ...
        [[maybe_unused]] auto create_component(const char* type,
                                               value_t component_count,
                                               bool create_2pc) -> std::string;

        // Create 2PC shards
        void create_2pc_shards(std::string default_log_level,
                               size_t current_component_num);
        // Create 2PC sentinels
        void create_2pc_sentinels(std::string default_log_level,
                                  size_t current_component_num);
        // Create 2PC coordinators
        void create_2pc_coordinators(std::string default_log_level,
                                     size_t current_component_num);

        // Create atomizer shards
        void create_atomizer_shards(std::string default_log_level,
                                    size_t current_component_num);
        // Create atomizer sentinels
        void create_atomizer_sentienls(std::string default_log_level,
                                       size_t current_component_num);
        // Create atomizer archivers
        void create_atomizer_archivers(std::string default_log_level,
                                       size_t current_component_num);
        // Create atomizer atomizers
        void create_atomizer_atomizers(std::string default_log_level,
                                       size_t current_component_num);
        // Create atomizer watchtowers
        void create_atomizer_whatchtowers(std::string default_log_level,
                                          size_t current_component_num);
        ...
        template<typename T>
        [[nodiscard]] auto find_value(std::string key) -> std::optional<T>;
    };
}
