...
namespace cbdc {
    ...
    auto operator<<(serializer& ser,
                    const threepc::runtime_locking_shard::shard_error& err)
        -> serializer&;
    auto operator>>(serializer& deser,
                    threepc::runtime_locking_shard::shard_error& err)
        -> serializer&;

    auto operator<<(serializer& ser,
                    const threepc::runtime_locking_shard::wounded_details& det)
        -> serializer&;
    auto operator>>(serializer& deser,
                    threepc::runtime_locking_shard::wounded_details& det)
        -> serializer&;

    auto operator<<(
        serializer& ser,
        const threepc::runtime_locking_shard::rpc::replicated_prepare_request&
            req) -> serializer&;
    auto operator>>(
        serializer& deser,
        threepc::runtime_locking_shard::rpc::replicated_prepare_request& req)
        -> serializer&;

    auto operator<<(serializer& ser,
                    const threepc::runtime_locking_shard::rpc::
                        replicated_get_tickets_request& req) -> serializer&;
    auto operator>>(
        serializer& deser,
        threepc::runtime_locking_shard::rpc::replicated_get_tickets_request&
            req) -> serializer&;
