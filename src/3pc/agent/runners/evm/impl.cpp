...
namespace cbdc::threepc::agent::runner {
    ...
    void evm_runner::do_run() {
        ...
        if(m_function.size() != 1) {
            ...
            // telemetry_log("evm_runner_run", 1, start);
            return;
        }
        ...
        if(f > static_cast<uint8_t>(
            ...
            m_result_callback(error_code::function_load);
            // telemetry_log("evm_runner_run", 2, start);
            return;
        }
        ...
        switch(evm_runner_function(f)) {
            case evm_runner_function::execute_transaction:
                // telemetry_log("evm_runner_run", 4, start);
        ...
        // telemetry_log("evm_runner_run", 3, start);
        ...
        auto success = m_try_lock_callback(
            ...
            [this](const broker::interface::try_lock_return_type& res) {
                if(!std::holds_alternative<broker::value_type>(res)) {
                    ...
                    m_result_callback(error_code::function_load);
                    // telemetry_log("evm_runner_run_get_account", 1, start);
                    return;
                }
                ...
                // telemetry_log("evm_runner_run_get_account", 0, start);
            });
        ...
        auto success = m_try_lock_callback(
            ...
            [this](const broker::interface::try_lock_return_type& res) {
                if(!std::holds_alternative<broker::value_type>(res)) {
                    ...
                    m_result_callback(error_code::function_load);
                    // telemetry_log("evm_runner_run_get_transaction_receipt",
                    //               1,
                    //               start);
                    return;
                }
            ...
            [this](const broker::interface::try_lock_return_type& res) {
                if(!std::holds_alternative<broker::value_type>(res)) {
                    m_log->error(
                        "Failed to read transaction receipt from shards");
                    m_result_callback(error_code::function_load);
                    // telemetry_log("evm_runner_run_get_transaction", 1,
                    // start);
                    return;
                }
                auto v = std::get<broker::value_type>(res);
                ...
                if(!maybe_receipt.has_value()) {
                    ...
                    m_result_callback(error_code::function_load);
                    // telemetry_log("evm_runner_run_get_transaction", 2,
                    // start);
                    return;
                }
                ret[m_param] = make_buffer(maybe_receipt.value().m_tx);
                m_result_callback(ret);
                // telemetry_log("evm_runner_run_get_transaction", 0, start);
            });
        ...
        auto success = m_try_lock_callback(
            ...
            [this](const broker::interface::try_lock_return_type& res) {
                if(!std::holds_alternative<broker::value_type>(res)) {
                    m_log->error("Failed to read account from shards");
                    m_result_callback(error_code::function_load);
                    // telemetry_log("evm_runner_run_get_account_code", 1,
                    // start);
                    return;
                }
                auto v = std::get<broker::value_type>(res);
                ...
                ret[m_param] = v;
                m_result_callback(ret);
                // telemetry_log("evm_runner_run_get_account_code", 0, start);
            });
    ...
    auto evm_runner::run_execute_real_transaction() -> bool {
        ...
        if(!maybe_tx.has_value()) {
            ...
            m_result_callback(error_code::function_load);
            // telemetry_log("evm_runner_run_execute_real_transaction", 1,
            // start);
            return true;
        }
        ...
        if(!maybe_from.has_value()) {
            ...
            // telemetry_log("evm_runner_run_execute_real_transaction", 2,
            // start);
            return true;
        }
        auto from = maybe_from.value();
        return run_execute_transaction(from, false);
        // telemetry_log("evm_runner_run_execute_real_transaction", 0, start);
    }
    ...
    auto evm_runner::run_execute_dryrun_transaction() -> bool {
        ...
        if(!maybe_tx.has_value()) {
            m_log->error("Unable to deserialize transaction");
            m_result_callback(error_code::function_load);
            // telemetry_log("evm_runner_run_execute_dryrun_transaction",
            //               1,
            //               start);
            return true;
        }
        ...
        // telemetry_log("evm_runner_run_execute_dryrun_transaction", 0,
        // start);
    }
    ...
    auto evm_runner::run_execute_transaction(const evmc::address& from,
        ...
        if(!enough_gas) {
            ...
            m_result_callback(error_code::exec_error);
            // telemetry_log("evm_runner_run_execute_transaction", 1, start);
            return true;
        }
        m_msg = msg;
        ...
        if(!dry_run) {
            ...
            auto r = m_try_lock_callback(
                ...
                [this](const broker::interface::try_lock_return_type& res) {
                    // telemetry_log("evm_runner_run_execute_transaction",
                    //               0,
                    //               start);
            ...
            if(!r) {
                ...
                m_result_callback(error_code::internal_error);
                // telemetry_log("evm_runner_run_execute_transaction", 2,
                // start);
                return false;
            }
        } else {
            schedule_exec();
            // telemetry_log("evm_runner_run_execute_transaction", 0, start);
        }
    ...
    void evm_runner::exec() {
        ...
        if(result.status_code < 0) {
            ...
            // telemetry_log("evm_runner_exec", 0, start);
        }
    }
    ...
    void evm_runner::handle_lock_from_account(
        ...
        if(!std::holds_alternative<broker::value_type>(res)) {
            ...
            m_result_callback(error_code::wounded);
            // telemetry_log("evm_runner_handle_lock_from_account", 1, start);
            return;
        }
        auto v = std::get<broker::value_type>(res);
        ...
        if(exp_nonce != m_tx.m_nonce) {
            m_log->error(m_ticket_number,
                         ...
                         to_hex(exp_nonce));
            m_result_callback(error_code::exec_error);
            // telemetry_log("evm_runner_handle_lock_from_account", 2, start);
            return;
        }
        ...
        if(from_acc.m_balance < required_funds) {
            ...
            m_result_callback(error_code::exec_error);
            // telemetry_log("evm_runner_handle_lock_from_account", 3, start);
            return;
        }
        ...
        auto maybe_sent = m_try_lock_callback(
            ...
            [this](const broker::interface::try_lock_return_type& r) {
                if(!std::holds_alternative<broker::value_type>(r)) {
                    ...
                    m_result_callback(error_code::wounded);
                    // telemetry_log("evm_runner_handle_lock_from_account",
                    //               4,
                    //               start);
                    return;
                }
                ...
                schedule_exec();
                // telemetry_log("evm_runner_handle_lock_from_account", 0,
                // start);
            });
        if(!maybe_sent) {
            m_log->error("Failed to send try_lock request for TX receipt");
            m_result_callback(error_code::internal_error);
            // telemetry_log("evm_runner_handle_lock_from_account", 5, start);
            return;
        }
    }
    ...
    void evm_runner::schedule_exec() {
        ...
        auto fn = [this]() {
            exec();
            // telemetry_log("evm_runner_schedule_exec", 0, start);
        };
        schedule(fn);
    }

    void evm_runner::telemetry_log(const cbdc::telemetry_key& func,
                                   uint8_t outcome,
                                   int64_t start) {
        if(!m_tel) {
            return;
        ...
        if(m_tx_set) {
            det.emplace_back(cbdc::telemetry_keys::txid, m_txid);
        }

        m_tel->log(func, det);
    }

    void evm_runner::schedule(std::function<void()> fn) {
        if(m_threads) {
            m_threads->push(fn);
            return;
        }
